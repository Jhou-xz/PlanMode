...
async def test_schedule_daily_summary_adds_cron_job(session):
    user = await get_or_create_user(session, discord_user_id="summary_user", discord_username="summary")
...
async def test_schedule_all_daily_summaries(session):
    user1 = await get_or_create_user(session, discord_user_id="all_summary_1", discord_username="all1")
    user2 = await get_or_create_user(session, discord_user_id="all_summary_2", discord_username="all2")
